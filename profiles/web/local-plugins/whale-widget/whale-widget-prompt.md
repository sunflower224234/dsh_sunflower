...
- **峰谷定价换算**：按每个小时桶的整点（北京时间 UTC+8）判定空闲/高峰，套用 `PRICING` 表（每百万 token 单价）求和：
  - 高峰时段：仅**工作日** 9:00–12:00 与 14:00–18:00。
  - **双休日（周六、周日）全天统一按低谷（空闲）价**，不参与工作日的分时高峰判定。
...
- 按 `data.turn` 聚合：同一 turn 的多步（step）usage 累加；成本换算复用峰谷定价表：`cacheRead→p.hit[off]`、`input→p.miss[off]`、`output+reasoning→p.out[off]`（off = 当前是否高峰，双休日全天为低谷/off=0）。
...
| 峰值判定 | 北京时间：工作日高峰 9–12 与 14–18 点；双休日（周六、周日）全天为低谷 |
